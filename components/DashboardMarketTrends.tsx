...
const marketTrendEndpoint = `${API_BASE_URL}${DAILY_MARKET_TRENDS}`;
...
function parseMarketTrend(value: unknown): MarketTrend | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  const product = isRecord(value.product) ? value.product : null;
  const offering = isRecord(value.productOffering) ? value.productOffering : null;
  const variant = isRecord(value.variantName) ? value.variantName : null;
  const brand = isRecord(value.brandName) ? value.brandName : undefined;
  const productId = readText(value, ["productId"]) || (product ? readText(product, ["id"]) : "");
  ...
  return {
    ...
    offeringSku: readText(value, ["offeringSku", "sku"]) || undefined,
    variantName: readText(value, ["variantName"]) || undefined,
    brandName: readText(value, ["brandName"]) || undefined,
    currentPrice: readNumber(value, ["currentPrice"]) ?? undefined,
    previousPrice: readNumber(value, ["previousPrice"]) ?? undefined,
    currency: readText(value, ["currency"]) || "NGN",
    changePercentage: readNumber(value, ["changePercentage"]) ?? undefined,
    trend: readText(value, ["trend"]) || undefined,
...
export function DashboardMarketTrends() {
  ...
  const [whatsAppMarketLabel, setWhatsAppMarketLabel] = useState("");
  const [whatsAppCopied, setWhatsAppCopied] = useState(false);
  const [bulkResult, setBulkResult] = useState<BulkUploadResult | null>(null);
  const [bulkStage, setBulkStage] = useState<"idle" | "validating" | "validated" | "committing" | "committed">("idle");
  const [isLoading, setIsLoading] = useState(true);
  const [trendAnalysisDate, setTrendAnalysisDate] = useState(new Date().toISOString());
  ...
  const WHATSAPP_BIG_MOVE_THRESHOLD = 20;
const WHATSAPP_ANOMALY_THRESHOLD = 200;
   const whatsAppMessage = useMemo(
    () => buildWhatsAppMessage(marketTrends, trendAnalysisDate, whatsAppMarketLabel.trim()),
    [marketTrends, trendAnalysisDate, whatsAppMarketLabel],
  );

  function combineProductLabel(productName: string, variantName?: string) {
  ...
  if (v.includes(p) || p.includes(v)) {
    return variantName.length >= productName.length ? variantName : productName;
  }

  return `${productName} ${variantName}`;
}

function formatWhatsAppPercent(value: number) {
  const sign = value > 0 ? "+" : "";
  const rounded = Math.abs(value) < 1 ? value.toFixed(1) : Math.round(value);
  return `${sign}${rounded}%`;
}



  function buildWhatsAppMessage(trends: MarketTrend[], analysisDate: string, marketLabel: string) {
  const usable = trends.filter(
    (trend) => trend.currentPrice !== undefined && trend.previousPrice !== undefined,
  );

  if (!usable.length) {
    return "No market trend data available yet.";
  }

  const currency = usable[0]?.currency || "NGN";
  const currencySymbol = currency === "NGN" ? "₦" : currency;

  const rows = usable.map((trend) => ({
    ...trend,
    changeNum: trend.changePercentage ?? 0,
    label: combineProductLabel(trend.productName, trend.variantName),
    pack: trend.packageName || trend.brandName || null,
  }));

  const gainers = rows
    .filter((row) => row.changeNum >= WHATSAPP_BIG_MOVE_THRESHOLD)
    .sort((a, b) => b.changeNum - a.changeNum);
  const drops = rows
    .filter((row) => row.changeNum <= -WHATSAPP_BIG_MOVE_THRESHOLD)
    .sort((a, b) => a.changeNum - b.changeNum);
  const minor = rows
    .filter((row) => Math.abs(row.changeNum) < WHATSAPP_BIG_MOVE_THRESHOLD)
    .sort((a, b) => Math.abs(b.changeNum) - Math.abs(a.changeNum));

  const renderLine = (row: (typeof rows)[number]) => {
    const name = row.pack ? `${row.label} (${row.pack})` : row.label;
    const anomaly = Math.abs(row.changeNum) >= WHATSAPP_ANOMALY_THRESHOLD ? " ⚠️" : "";
    return `${name}: ${formatNumber(row.previousPrice ?? 0)} → ${formatNumber(row.currentPrice ?? 0)} (${formatWhatsAppPercent(row.changeNum)})${anomaly}`;
  };

  const lines: string[] = [
    `📊 *Ojaboy Market Update — ${formatDate(analysisDate)}*`,
    marketLabel ? `${marketLabel} · Prices in ${currencySymbol}` : `Prices in ${currencySymbol}`,
  ];

  if (gainers.length) {
    lines.push("", "🔺 *Big Hike*", ...gainers.map(renderLine));
  }
  if (drops.length) {
    lines.push("", "🔻 *Big Drops*", ...drops.map(renderLine));
  }
  if (minor.length) {
    lines.push("", "↔️ *Minor moves*", ...minor.map(renderLine));
  }
  if (rows.some((row) => Math.abs(row.changeNum) >= WHATSAPP_ANOMALY_THRESHOLD)) {
    lines.push("", "⚠️ = unusually large swing, worth double-checking before sending.");
  }
  ...
  const loadMarketTrends = useCallback(async () => {
    try {
      const response = await authenticatedFetch(marketTrendEndpoint, {
        headers: { Accept: "application/json" },
      });
      const body = (await response.json().catch(() => null)) as unknown;
      
        if (response.ok && isRecord(body) && isRecord(body.snapshot)) { 
          const snapshot = body.snapshot;
          setMarketTrends(parseList(snapshot?.items, "marketTrends", parseMarketTrend));
           setTrendAnalysisDate(readText(snapshot, ["analysisDate"]) || new Date().toISOString());
      }
    } catch {
      setMarketTrends([]);
    }finally{
  ...
  async function downloadTemplate() {
   ...
   async function handleCopyWhatsApp() {
    try {
      ...
      setNotice("Market update copied — paste it into WhatsApp.");
      setTimeout(() => setWhatsAppCopied(false), 2000);
    } catch {
      setError("Unable to copy to clipboard. Select and copy the text manually instead.");
    }
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-black tracking-normal text-black">Market Trends</h1>
          <p className="mt-2 text-sm font-medium text-black/58">Daily market trends are available for viewing.</p>
        </div>
         <div className="flex flex-wrap items-center gap-3">
            <button
              className="flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-black text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50"
              disabled={!marketTrends.length}
              type="button"
      ...
      <section className="overflow-hidden rounded-xl border border-black/10 bg-white shadow-[0_14px_35px_rgba(0,0,0,0.04)]">
        <div className="grid grid-cols-6 gap-4 bg-[#fff5f5] px-5 py-4 text-xs font-black uppercase text-black/50">
        <span>Product</span>
        <span>Trend</span>
        ...
        {isLoading ? (
          ...
          marketTrends.map((trend) => (
            <article
              className="grid grid-cols-6 items-center gap-4 border-t border-black/10 px-5 py-4 text-sm"
              key={trend.id}
            >
              <div className="min-w-0">
                <p className="truncate font-black text-black">{trend.offeringSku || trend.productName}</p>
                <p className="mt-1 truncate text-xs font-medium text-black/50">
                  {[trend.productName, trend.variantName, trend.brandName].filter(Boolean).join(" · ") || trend.productId}
                  {trend.variantName || trend.brandName ? ` (${trend.productId})` : ""}
                </p>
              </div>
              <p className="font-bold text-black/64">{trend.trend === "up" ? <TrendingUp size={16} color="red"/> : trend.trend === "down" ? <TrendingDown size={16} color="green"/> : "-"}</p>
              <p className="font-bold text-black/64">{trend.changePercentage !== undefined ? `${trend.changePercentage.toFixed(2)}%` : "N/A"}</p>
              <p className="font-black text-black">{trend.previousPrice !== undefined ? formatMoney(trend.previousPrice, trend.currency) : "N/A"}</p>
              <p className="font-black text-black">{trend.currentPrice !== undefined ? formatMoney(trend.currentPrice, trend.currency) : "N/A"}</p>
              <p className="text-xs font-bold text-black/45">{formatDate(trend.lastPriceAt)}</p>
      ...
      {isBulkOpen ? (
        ...
        >
          <section
            aria-labelledby="market-trend-whatsapp-title"
            aria-modal="true"
            className="max-h-[90dvh] w-full max-w-lg overflow-y-auto rounded-t-[1.5rem] bg-white p-5 shadow-2xl sm:rounded-[1.5rem]"
            role="dialog"
          >
            <header className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-lg font-black text-black" id="market-trend-whatsapp-title">Share Market Update</h2>
                <p className="mt-1 text-xs font-medium text-black/48">Review the message, then copy it into your WhatsApp group.</p>
              </div>
              <button
            ...
            <label className="mt-4 block">
              <span className="text-xs font-black text-black">Market label (optional)</span>
              <input
                className="mt-2 h-11 w-full rounded-xl border border-black/10 bg-[#fafafa] px-4 text-sm outline-none focus:border-[#f10606]/45"
                placeholder="e.g. Ile Epo Market"
                value={whatsAppMarketLabel}
                onChange={(event) => setWhatsAppMarketLabel(event.target.value)}
              />
            </label>

            <div className="mt-4 rounded-xl border border-black/10 bg-[#fafafa] p-4">
              <pre className="whitespace-pre-wrap break-words font-sans text-sm text-black">{whatsAppMessage}</pre>
            </div>

            <button
              className="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 text-sm font-black text-white transition hover:bg-emerald-700"
              type="button"
